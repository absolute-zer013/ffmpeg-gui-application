  ...
  static Future<ExportResult> exportFile({
    ...
    if (item.codecSettings.isNotEmpty) {
      ...
      for (final entry in item.codecSettings.entries) {
        ...
        final videoTrack = item.videoTracks.where((t) => t.streamIndex == streamIndex).firstOrNull;
        final audioTrack = item.audioTracks.where((t) => t.streamIndex == streamIndex).firstOrNull;
      ...
      if (videoSettings.isNotEmpty) {
        ...
        final allSameCodec = videoSettings.values.map((s) => s.videoCodec).toSet().length == 1;
        if (allSameCodec && videoSettings.length == item.selectedVideo.length) {
          ...
          for (final entry in videoSettings.entries) {
            ...
            if (settings.videoCodec != null) {
              args.addAll(['-c:v:$streamIndex', settings.videoCodec!.ffmpegName]);
            }
          }
        }
      }
      ...
      if (audioSettings.isNotEmpty) {
        for (final entry in audioSettings.entries) {
          ...
          if (settings.audioChannels != null) {
            args.addAll(['-ac:$streamIndex', settings.audioChannels.toString()]);
          }
          if (settings.audioSampleRate != null) {
            args.addAll(['-ar:$streamIndex', settings.audioSampleRate.toString()]);
          }
        }
      }
    }
